...
- 자세(`wait`)를 잡고 0.9초 뒤 옆모습(발도술)으로 겹쳐 넘어간다(`samProfileK`) — Y자는 그 뒤에 열린다
- 기술을 고르면 그 사람부터 칼을 뽑기 시작한다(`m.drawAt[id]`), 상대 칼은 판정이 올 때 나온다. Y자는 `locked` 로 스르르 걷힌다
- 스쳐 베기 `READY`(620ms)는 칼 뽑는 데 걸리는 시간이기도 하다. 준비 컷(`drawSamReady`) — 발도술 자세로 마주 서서 손 · 자루(`sam_iai_hand`)가 칼집 입에서 앞으로 나가고 그 사이로 날이 드러난다(`samIaiDraw`). 빛이 날을 훑고 → 칼집 입에서 번뜩. 소리는 `swordout`
